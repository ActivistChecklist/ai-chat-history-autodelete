...
(function () {
  ...
  /** Path `d` values from Simple Icons (CC0): https://github.com/simple-icons/simple-icons */
  var ICON_PATH = {
    googlechrome:
  ...
  var BROWSER = {
    chrome: { label: 'Add to Chrome', icon: 'googlechrome' },
    edge: { label: 'Add to Edge', icon: 'microsoftedge' },
    brave: { label: 'Add to Brave', icon: 'brave' },
    opera: { label: 'Add to Opera', icon: 'opera' },
    vivaldi: { label: 'Add to Vivaldi', icon: 'vivaldi' },
    arc: { label: 'Add to Arc', icon: 'arc' },
    yandex: { label: 'Add to Yandex Browser', icon: 'googlechrome' },
    chromium: { label: 'Add to Chromium', icon: 'googlechrome' },
  };
  ...
  function brandId(brand) {
    ...
    if (b.includes('yandex')) return 'yandex';
    return null;
  }

  function detectFromBrands(brands) {
    if (!brands || !brands.length) return null;
    var i;
    for (i = 0; i < brands.length; i++) {
      var id = brandId(brands[i].brand);
      if (id) return id;
    }
    var hasChromium = brands.some(function (x) {
      return String(x.brand).toLowerCase().includes('chromium');
    });
    var hasChrome = brands.some(function (x) {
      return String(x.brand).toLowerCase().includes('google chrome');
    });
    if (hasChromium && !hasChrome) return 'chromium';
    return null;
  }

  function detectFromUA(ua) {
    if (/Edg\//.test(ua)) return 'edge';
    if (/OPR\//.test(ua) || /\bOpera\b/i.test(ua)) return 'opera';
    if (/Vivaldi/i.test(ua)) return 'vivaldi';
    if (/\bArc\b/i.test(ua)) return 'arc';
    if (/YaBrowser/i.test(ua)) return 'yandex';
    if (/\bBrave\b/i.test(ua)) return 'brave';
    if (/\bChromium\b/i.test(ua) && !/Edg\//.test(ua)) return 'chromium';
    return 'chrome';
  }
  ...
  function syncDetect() {
    ...
    var fromBrands = detectFromBrands(brands);
    if (fromBrands) return fromBrands;
    return detectFromUA(ua);
  }

  function apply(id) {
    var cfg = BROWSER[id] || BROWSER.chrome;
    var d = ICON_PATH[cfg.icon] || ICON_PATH.googlechrome;
    var link = document.getElementById('store-cta');
    var pathEl = document.getElementById('store-cta-path');
    var labelEl = document.getElementById('store-cta-label');
    if (!link || !pathEl || !labelEl) return;
  ...
  function run() {
    var first = syncDetect();
    apply(first);

    if (first === 'chrome' && navigator.brave && typeof navigator.brave.isBrave === 'function') {
      navigator.brave
        .isBrave()
        .then(function (isBrave) {
          if (isBrave) apply('brave');
        })
        .catch(function () {});
    }
  }
